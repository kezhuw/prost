...
impl<'a> CodeGenerator<'a> {
    ...
    fn append_field(&mut self, fq_message_name: &str, field: FieldDescriptorProto) {
        ...
        let required = self.required(fq_message_name, &field);
        let optional = !required && self.optional(&field);
        ...
        let type_tag = self.field_type_tag(fq_message_name, &field);
    ...
    fn append_oneof(
        ...
        for (field, idx) in fields {
            ...
            self.push_indent();
            let ty_tag = self.field_type_tag(fq_message_name, &field);
            self.buf.push_str(&format!(
                "#[prost({}, tag=\"{}\")]\n",
                ty_tag,
                field.number()
